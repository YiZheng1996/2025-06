...
namespace MainUI.Procedure
{
    public partial class ucTypeManage : ucBaseManagerUI
    {
        ...
        private void LoadModelType()
        {
            ...
            cboModelName.DataSource = BLLmodelType.GetAllModelType();
        }

        private void LoadModel(int typeid)
        {
            dataGridView1.DataSource = bll.GetAllKind(typeid);
            txtTypeName.Text = "";
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (bll.IsExist(typeid, curModel))
                {
            ...
            {
                MessageHelper.MessageOK($"新增失败：{ex.Message}");
            }
        }
        ...
        private void btnChange_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string curModel = txtTypeName.Text.Trim();
                if (string.IsNullOrEmpty(curModel)) return;
                int typeid = Convert.ToInt32(cboModelName.SelectedValue.ToString());
                string mark = txtMark.Text;
        ...
        private void cboModelType_SelectedIndexChanged(object sender, EventArgs e)
        {
            txtTypeName.Text = "";
            txtMark.Text = "";
            id = cboModelName.SelectedValue.ToInt32();
            LoadModel(id);
