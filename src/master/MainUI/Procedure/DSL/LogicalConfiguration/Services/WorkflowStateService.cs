    ...
    public class WorkflowStateService : IWorkflowStateService, IDisposable
    {
        ...
        private void OnStepNumChanged(int newStepNum)
        {
            lock (_eventLock)
            {
                ...
                catch (Exception ex)
                {
                    // 记录异常但不重新抛出
                    // 避免事件处理程序的异常影响主程序流程
                    System.Diagnostics.Debug.WriteLine($"StepNumChanged 事件处理异常: {ex.Message}");
        ...
        private void OnVariableAdded(object variable)
        {
            lock (_eventLock)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"VariableAdded 事件处理异常: {ex.Message}");
                }
            }
        }
        ...
        private void OnVariableRemoved(object variable)
        {
            lock (_eventLock)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"VariableRemoved 事件处理异常: {ex.Message}");
                }
            }
        }
        ...
        private void OnStepAdded(ChildModel step)
        {
            lock (_eventLock)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"StepAdded 事件处理异常: {ex.Message}");
                }
            }
        }
        ...
        private void OnStepRemoved(ChildModel step)
        {
            lock (_eventLock)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"StepRemoved 事件处理异常: {ex.Message}");
                }
            }
        }
        ...
        private void Dispose(bool disposing)
        {
            if (!_disposed && disposing)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"WorkflowStateService 释放资源时发生异常: {ex.Message}");
                }
                finally
                {
                    _disposed = true;
